...
- **Conversational memory:** multi-turn follow-ups are rewritten into standalone queries (*condense-question*) from a bounded last-N + running-summary history (contract-scoped). This fixes **retrieval**, not just generation — a pronoun-laden follow-up otherwise retrieves the wrong chunks — and grounding is untouched because the standalone query flows through the same gate→retrieve→verify path.
...
**Production (1,000 concurrent queries):** bottlenecks are (a) embedding/rerank CPU → GPU batch server or lighter model; (b) LLM API rate limits → response caching + request queue; (c) FAISS flat → HNSW/pgvector. **Measured per-query latency** (logged for every query): retrieval ~0.2s CPU · generation ~3s (Haiku 4.5, ~1.2k input / ~0.2k output tokens) · query gate ~2.6s (parallelizable with retrieval) → ~6s total; Bedrock prompt-caching amortizes repeated per-contract context across a session. **Freshness (demonstrated):** adding a new contract embeds only that contract — 0.8s, existing vectors untouched, immediately queryable (`results/freshness_demo.md`); the real re-embed trigger is an embedding-model version change, not corpus growth.
